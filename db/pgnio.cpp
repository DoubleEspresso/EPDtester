...
bool pgn_io::parse(Board& b)
{
	...
	if (!ifile->is_open())
	{
		Log::write("..ERROR: failed to open pgn file, check filename.\n");
		return false;
	}
	...
	while (std::getline(*ifile, line))
	{
		...
		else if (line.size() > 0 && line != "\n")
		{
			...
			if (eog)
			{
				...
				{
					Log::write("..end pgn parse, space full\n");
					Log::write("..%d games, %d moves, %d collisions parsed successfully.", games, move_count, collision_count);
					write();
					return true;
				}
	...
	//printf("header tag --> %s\n", data->tags[j].c_str());
	write();
	Log::write("..finished, %d games, %d moves, %d collisions parsed successfully.", games, move_count, collision_count);
	return true;
}
...
{
	std::string result = "";
	if (!ofile) return result;
	if (!book->compute_key(fen)) return result;
	U64 k = book->key();
	...
	if (e->key == k)
	{
		...
		{
			char tmp[256];
			sprintf(tmp, "%s %d %d %d\n", responses[j].c_str(), results[j][1], results[j][0], results[j][2]); // white wins - draws - black wins
...
bool pgn_io::write()
{
	...
	Log::write("..finished writing %d moves to db file\n", count);
	ofile->close();
